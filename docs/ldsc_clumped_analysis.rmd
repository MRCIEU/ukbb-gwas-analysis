...
clumped$mac <- clumped$maf^2 * clumped$ncase
```

How many do we keep for different mac cutoffs?

```{r}
tibble(mac_threshold = seq(1,100,by=1)) %>%
	group_by(mac_threshold) %>%
	summarise(remaining_hits = sum(clumped$mac > mac_threshold)) %>%
ggplot(., aes(x=mac_threshold, y=remaining_hits)) +
geom_point() +
geom_vline(xintercept=10)
```

There are `r sum(clumped$mac < 1)` hits with expected mac < 1. So, that's where the vast majority of hits are coming from. After that, there's not really much difference in terms of number of associations when mac > 1 and mac > 100. So, maybe take the threshold of mac > 10?

This would leave `r sum(clumped$mac > 10)` GWAS hits.


What studies are coming up? e.g. how many cases in binary etc?

```{r}
clumped$keep <- clumped$mac > 10
nclumped <- clumped %>%
group_by(id) %>%
